...
Examples:
  npx claude-flow@alpha hook pre-edit --file "src/auth/login.js"
  npx claude-flow@alpha hook pre-edit -f "config/db.js" --validate-syntax
  npx claude-flow@alpha hook pre-edit -f "production.env" --backup-file --check-conflicts
```
...
```bash
npx claude-flow@alpha hook pre-bash --command <cmd>
...
Examples:
  npx claude-flow@alpha hook pre-bash -c "rm -rf /tmp/cache"
  npx claude-flow@alpha hook pre-bash --command "docker build ." --estimate-resources
```
...
Examples:
  npx claude-flow@alpha hook pre-task --description "Implement user authentication"
  npx claude-flow@alpha hook pre-task -d "Continue API dev" --load-memory
  npx claude-flow@alpha hook pre-task -d "Refactor codebase" --optimize-topology
```
...
```bash
npx claude-flow@alpha hook pre-search --query <query>
...
Examples:
  npx claude-flow@alpha hook pre-search -q "authentication middleware"
```
...
Examples:
  npx claude-flow@alpha hook post-edit --file "src/components/Button.jsx"
  npx claude-flow@alpha hook post-edit -f "api/auth.js" --memory-key "auth/login"
  npx claude-flow@alpha hook post-edit -f "utils/helpers.ts" --train-patterns
```
...
```bash
npx claude-flow@alpha hook post-bash --command <cmd>
...
Examples:
  npx claude-flow@alpha hook post-bash -c "npm test" --update-metrics
```
...
Examples:
  npx claude-flow@alpha hook post-task --task-id "auth-implementation"
  npx claude-flow@alpha hook post-task -t "api-refactor" --analyze-performance
  npx claude-flow@alpha hook post-task -t "bug-fix-123" --store-decisions
```
...
```bash
npx claude-flow@alpha hook post-search --query <query> --results <path>
...
Examples:
  npx claude-flow@alpha hook post-search -q "auth" -r "results.json" --train-patterns
```
...
```bash
npx claude-flow@alpha hook mcp-initialized --swarm-id <id>
...
```bash
npx claude-flow@alpha hook agent-spawned --agent-id <id> --type <type>
...
```bash
npx claude-flow@alpha hook task-orchestrated --task-id <id>
...
```bash
npx claude-flow@alpha hook neural-trained --pattern <name>
...
```bash
npx claude-flow@alpha hook memory-sync --namespace <ns>
...
```bash
npx claude-flow@alpha hook session-start --session-id <id>
...
```bash
npx claude-flow@alpha hook session-restore --session-id <id>
...
Examples:
  npx claude-flow@alpha hook session-restore --session-id "swarm-20241019"
  npx claude-flow@alpha hook session-restore -s "feature-auth" --restore-memory
```
...
Examples:
  npx claude-flow@alpha hook session-end --session-id "dev-session-2024"
  npx claude-flow@alpha hook session-end -s "feature-auth" --export-metrics --generate-summary
  npx claude-flow@alpha hook session-end -s "quick-fix" --cleanup-temp
```
...
```bash
npx claude-flow@alpha hook notify --message <msg>
...
Examples:
  npx claude-flow@alpha hook notify -m "Task completed" --level info
  npx claude-flow@alpha hook notify -m "Critical error" --level error --broadcast
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx claude-flow@alpha hook pre-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/current'"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx claude-flow@alpha hook pre-bash --command '${tool.params.command}'"
        }]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "npx claude-flow@alpha hook post-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/complete' --auto-format --train-patterns"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "npx claude-flow@alpha hook post-bash --command '${tool.params.command}' --update-metrics"
        }]
      }
    ]
  }
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx claude-flow@alpha hook pre-edit --file '${tool.params.file_path}' --auto-assign-agent --validate-syntax",
            "timeout": 3000,
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx claude-flow@alpha hook pre-task --description '${tool.params.task}' --auto-spawn-agents --load-memory",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx claude-flow@alpha hook pre-search --query '${tool.params.pattern}' --check-cache"
          }
        ]
      }
    ],
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx claude-flow@alpha hook post-edit --file '${tool.params.file_path}' --memory-key 'edits/${tool.params.file_path}' --auto-format --train-patterns",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx claude-flow@alpha hook post-task --task-id '${result.task_id}' --analyze-performance --store-decisions --export-learnings",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx claude-flow@alpha hook post-search --query '${tool.params.pattern}' --cache-results --train-patterns"
          }
        ]
      }
    ],
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx claude-flow@alpha hook session-start --session-id '${session.id}' --load-context"
          }
        ]
      }
    ],
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "npx claude-flow@alpha hook session-end --session-id '${session.id}' --export-metrics --generate-summary --cleanup-temp"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx claude-flow@alpha hook check-protected --file '${tool.params.file_path}'"
          }
        ]
      }
    ]
...
```javascript
// Hook command
npx claude-flow@alpha hook pre-task --description "Build REST API"
...
```javascript
// Hook command
npx claude-flow@alpha hook post-edit --file "api/auth.js"
...
```javascript
// Hook command
npx claude-flow@alpha hook session-end --session-id "dev-2024"
...
for FILE in $FILES; do
  ...
  npx claude-flow@alpha hook pre-edit --file "$FILE" --validate-syntax
  ...
  npx claude-flow@alpha hook post-edit --file "$FILE" --auto-format
done
...
npx claude-flow@alpha hook notify \
...
npx claude-flow@alpha hook pre-edit \
  --file "api/auth.js" \
  --auto-assign-agent \
  --validate-syntax
...
npx claude-flow@alpha hook session-restore \
...
npx claude-flow@alpha hook session-start --session-id "fullstack-feature"
...
# Backend work
npx claude-flow@alpha hook pre-edit --file "api/profile.js"
# ... implement backend ...
npx claude-flow@alpha hook post-edit \
  --file "api/profile.js" \
...
npx claude-flow@alpha hook pre-edit --file "components/Profile.jsx"
# ... implement frontend ...
npx claude-flow@alpha hook post-edit \
...
npx claude-flow@alpha hook session-start --session-id "debug-memory-leak"
...
npx claude-flow@alpha hook pre-search --query "EventEmitter"
# ... search executes ...
npx claude-flow@alpha hook post-search \
  --query "EventEmitter" \
  --cache-results
...
npx claude-flow@alpha hook pre-task --description "Analyze code complexity"
# ... analysis ...
npx claude-flow@alpha hook post-task \
  --task-id "analysis" \
  --store-decisions
...
for file in src/**/*.js; do
  npx claude-flow@alpha hook pre-edit --file "$file" --backup-file
  # ... refactor ...
  npx claude-flow@alpha hook post-edit \
    --file "$file" \
...
npx claude-flow@alpha hook pre-edit --file "test.js" --debug

# Check hook execution logs
cat .claude-flow/logs/hooks-$(date +%Y-%m-%d).log

# Validate configuration
npx claude-flow@alpha hook validate-config
```
...
- `npx claude-flow@alpha init --hooks` - Initialize hooks system
- `npx claude-flow@alpha hook --list` - List available hooks
- `npx claude-flow@alpha hook --test <hook>` - Test specific hook
- `npx claude-flow@alpha memory usage` - Manage memory
