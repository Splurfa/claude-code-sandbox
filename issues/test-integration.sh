...
# Tests issue detection, pattern tracking, and threshold triggering
...
# Test directories
TEST_DIR="/tmp/issue-tracking-tests-$$"
BACKUP_PATTERN_DB=""

echo -e "${BLUE}=== Issue Tracking Integration Test Suite ===${NC}"
echo ""
...
setup_test_env() {
    ...
    if [ -f "sessions/issues/.pattern-database.json" ]; then
        BACKUP_PATTERN_DB="sessions/issues/.pattern-database.json.backup-$$"
        cp "sessions/issues/.pattern-database.json" "$BACKUP_PATTERN_DB"
...
cleanup_test_env() {
    ...
    if [ -n "$BACKUP_PATTERN_DB" ] && [ -f "$BACKUP_PATTERN_DB" ]; then
        mv "$BACKUP_PATTERN_DB" "sessions/issues/.pattern-database.json"
...
test_single_session_pattern() {
    ...
    # Run detection
    bash sessions/issues/detect-issues.sh "$test_session" > /dev/null 2>&1 || true

    # Check pattern database
    local pattern_count=$(cat sessions/issues/.pattern-database.json | jq -r '.["file-routing-violation"].occurrences // 0')
...
test_threshold_triggering() {
    ...
    local initial_issue_count=$(ls -1 sessions/issues/ISSUE-*.md 2>/dev/null | wc -l | tr -d ' ')
    ...
    local final_issue_count=$(ls -1 sessions/issues/ISSUE-*.md 2>/dev/null | wc -l | tr -d ' ')
    local issues_created=$((final_issue_count - initial_issue_count))

    # Check threshold_reached flag
    local threshold_reached=$(cat sessions/issues/.pattern-database.json | jq -r '.["file-routing-violation"].threshold_reached // false')
    local issue_created=$(cat sessions/issues/.pattern-database.json | jq -r '.["file-routing-violation"].issue_created // "null"')

    if [ "$pattern_count" = "3" ] && [ "$threshold_reached" = "true" ] && [ "$issue_created" != "null" ]; then
        report_test "Threshold triggering" "pass" "Pattern count = 3, threshold reached, issue created: $issue_created"
    else
        report_test "Threshold triggering" "fail" "Pattern count = $pattern_count, threshold = $threshold_reached, issue = $issue_created"
    fi
...
test_multiple_patterns() {
    ...
    local pattern_count=$(cat sessions/issues/.pattern-database.json | jq 'keys | length')
    local has_routing=$(cat sessions/issues/.pattern-database.json | jq -r '.["file-routing-violation"] // "null"')
    local has_naming=$(cat sessions/issues/.pattern-database.json | jq -r '.["session-naming-violation"] // "null"')
...
test_pattern_database_functions() {
    ...
    # Test get pattern
    local pattern=$(bash sessions/issues/pattern-database.sh get "test-pattern")
    ...
    local count=$(cat sessions/issues/.pattern-database.json | jq -r '.["test-pattern"].occurrences // 0')
    ...
    # Test list
    local list_output=$(bash sessions/issues/pattern-database.sh list 2>/dev/null)
    ...
    # Test stats
    local stats_output=$(bash sessions/issues/pattern-database.sh stats 2>/dev/null)
...
test_detection_integration() {
    ...
    # Create session with multiple issues
...
EOF
    ...
    local output=$(bash sessions/issues/detect-issues.sh "$test_session" 2>&1)

    # Check if detection ran
    if echo "$output" | grep -q "Issue Detection Report"; then
    ...
    local db_content=$(cat sessions/issues/.pattern-database.json)
...
test_error_handling() {
    ...
    local output=$(bash sessions/issues/detect-issues.sh 2>&1 || true)
    ...
    local output=$(bash sessions/issues/detect-issues.sh "non-existent-session" 2>&1 || true)
...
test_edge_cases() {
    ...
    echo "invalid json {" > sessions/issues/.pattern-database.json

    # Should gracefully handle and reinitialize
    bash sessions/issues/pattern-database.sh store "test" "Test" "session" "low" "3" > /dev/null 2>&1 || true

    local db_valid=$(cat sessions/issues/.pattern-database.json | jq -r 'keys | length' 2>/dev/null || echo "0")
