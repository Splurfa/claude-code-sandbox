...
const { Context7Integration } = require('./context-aware-secure');
...
class EnhancedPromptAnalyzer {
   ...
   * @param {object} sanitizedContext - Sanitized context from PromptSanitizer
   * @param {object} options - Analysis options
   */
  async analyzeSecure(sanitizedContext, options = {}) {
    // Use safe text for analysis
    const prompt = sanitizedContext.safeText;
    
    // Call standard analysis
    const analysis = await this.analyze(prompt, options);
    
    // Attach security context
    analysis.securityContext = sanitizedContext.security;
