...
│  .swarm/memory.db (~100K entries, actively growing)            │
...
- **Rationale**: ~100K entries (actively growing) with <10ms lookups, portable across systems
- **Consequences**: WAL file can grow large (100-200MB, fluctuates with activity), requires periodic checkpointing
...
> *Statistics reflect typical ranges in active workspaces. Exact values vary with current activity.*
...
- ~100K memory entries (actively growing)
- 40-50 namespaces (typical range)
- 100-200MB WAL (fluctuates with activity)
...
**50+ documented agent types** (22 base definitions with extensibility to 80+ combinations)

Reference: [Agent Catalog](../../../docs/reference/agent-catalog.md)
  ...
  "bottlenecks": [
    {
      ...
      "issue": "WAL size 100-200MB (fluctuates) causing checkpoint delays",
